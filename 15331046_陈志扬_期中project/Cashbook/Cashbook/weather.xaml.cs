        ...
        async void queryAsyncXML(string city)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    weatherInfo.Text = "今" + node1.InnerText + " " + node2.InnerText + " " + node3.InnerText + "\n" +
                        "明" + node4.InnerText + " " + node5.InnerText + " " + node6.InnerText;
                    dressing.Text = node0.InnerText;
                    if (node2.InnerText.Contains("雨"))
                    {
                        sunny.Visibility = Visibility.Collapsed;
                        cloudy.Visibility = Visibility.Collapsed;
                        rainy.Visibility = Visibility.Visible;
                    }
                    else if (node2.InnerText.Contains("多云") || node2.InnerText.Contains("阴"))
                    {
                        sunny.Visibility = Visibility.Collapsed;
                        cloudy.Visibility = Visibility.Visible;
                        rainy.Visibility = Visibility.Collapsed;
                    }
                    else if (node2.InnerText.Contains("晴"))
                    {
